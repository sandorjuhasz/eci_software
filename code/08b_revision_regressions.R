### regressions for revision and response letter crafting
### by sandorjuhasz
...
### GDP instead of GDP per capita in Gini and Emissions regression
...
key_columns <- c("gini_2020_2022_norm", "log_gdp_ppp", "eci_software_norm", "eci_trade_norm", "eci_tech_norm", "eci_research_norm", "log_pop", "log_nat_res", "sim_eci_software_norm")
...
gini_m02 <- feols(gini_2020_2022_norm ~ eci_trade_norm + log_gdp_ppp + log_pop + log_nat_res, vcov = "HC1", data = reg_df)
gini_m03 <- feols(gini_2020_2022_norm ~ eci_tech_norm + log_gdp_ppp + log_pop + log_nat_res, vcov = "HC1", data = reg_df)
gini_m04 <- feols(gini_2020_2022_norm ~ eci_research_norm + log_gdp_ppp + log_pop + log_nat_res, vcov = "HC1", data = reg_df)
gini_m05 <- feols(gini_2020_2022_norm ~ eci_software_norm + log_gdp_ppp + eci_trade_norm + log_pop + log_nat_res, vcov = "HC1", data = reg_df)
gini_m06 <- feols(gini_2020_2022_norm ~ eci_software_norm + log_gdp_ppp + eci_tech_norm + log_pop + log_nat_res, vcov = "HC1", data = reg_df)
gini_m07 <- feols(gini_2020_2022_norm ~ eci_software_norm + log_gdp_ppp + eci_research_norm + log_pop + log_nat_res, vcov = "HC1", data = reg_df)
...
gini_ivm01 <- feols(gini_2020_2022_norm ~ 1 | eci_software_norm + log_gdp_ppp + log_pop + log_nat_res ~ sim_eci_software_norm + log_gdp_ppp + log_pop + log_nat_res, vcov = "HC1", data = reg_df)
gini_ivm08 <-
  feols(
    gini_2020_2022_norm ~ 1 |
      eci_software_norm + log_gdp_ppp + eci_trade_norm + eci_tech_norm + eci_research_norm + log_pop + log_nat_res ~ sim_eci_software_norm + log_gdp_ppp + eci_trade_norm + eci_tech_norm + eci_research_norm + log_pop + log_nat_res,
    vcov = "HC1",
    data = reg_df
  )
...
reg_df <- subset(df, year==2020)
reg_df$sim_eci_software_norm <- scale(reg_df$avg_eci_similar_spec)
key_columns <- c("log_emission_per_gdp", "log_gdp_ppp", "eci_software_norm", "eci_trade_norm", "eci_tech_norm", "eci_research_norm", "log_pop", "log_nat_res", "sim_eci_software_norm")
reg_df <- reg_df[complete.cases(reg_df[, ..key_columns]), ]

em_m01 <- feols(log_emission_per_gdp ~ eci_software_norm + log_gdp_ppp + log_pop + log_nat_res, vcov = "HC1", data = reg_df)
em_m02 <- feols(log_emission_per_gdp ~ eci_trade_norm + log_gdp_ppp + log_pop + log_nat_res, vcov = "HC1", data = reg_df)
em_m03 <- feols(log_emission_per_gdp ~ eci_tech_norm + log_gdp_ppp + log_pop + log_nat_res, vcov = "HC1", data = reg_df)
em_m04 <- feols(log_emission_per_gdp ~ eci_research_norm + log_gdp_ppp + log_pop + log_nat_res, vcov = "HC1", data = reg_df)
em_m05 <- feols(log_emission_per_gdp ~ eci_software_norm + eci_trade_norm + log_gdp_ppp + log_pop + log_nat_res, vcov = "HC1", data = reg_df)
em_m06 <- feols(log_emission_per_gdp ~ eci_software_norm + eci_tech_norm + log_gdp_ppp + log_pop + log_nat_res, vcov = "HC1", data = reg_df)
em_m07 <- feols(log_emission_per_gdp ~ eci_software_norm + eci_research_norm + log_gdp_ppp + log_pop + log_nat_res, vcov = "HC1", data = reg_df)
em_m08 <- feols(log_emission_per_gdp ~ eci_software_norm + eci_trade_norm + eci_tech_norm + eci_research_norm + log_gdp_ppp + log_pop + log_nat_res, vcov = "HC1", data = reg_df)
em_ivm01 <- feols(log_emission_per_gdp ~ 1 | eci_software_norm + log_gdp_ppp + log_pop + log_nat_res ~ sim_eci_software_norm + log_gdp_ppp + log_pop + log_nat_res, vcov = "HC1", data = reg_df)
em_ivm08 <-
  feols(
    log_emission_per_gdp ~ 1 |
      eci_software_norm + eci_trade_norm + eci_tech_norm + eci_research_norm + log_gdp_ppp + log_pop + log_nat_res ~ sim_eci_software_norm + eci_trade_norm + eci_tech_norm + eci_research_norm + log_gdp_ppp + log_pop + log_nat_res,
    vcov = "HC1",
    data = reg_df
  )
...
reg_df <- subset(df, year==2020)
reg_df$sim_eci_software_norm <- scale(reg_df$avg_eci_similar_spec)
key_columns <- c("gini_norm", "log_gdp_ppp_pc", "eci_software_norm", "eci_trade_norm", "eci_tech_norm", "eci_research_norm", "log_pop", "log_nat_res", "sim_eci_software_norm")
reg_df <- reg_df[complete.cases(reg_df[, ..key_columns]), ]
...
gini_ivm01 <- feols(gini_norm ~ 1 | eci_software_norm + log_gdp_ppp_pc + log_pop + log_nat_res ~ sim_eci_software_norm + log_gdp_ppp_pc + log_gdp_ppp_pc2 + log_pop + log_nat_res, vcov = "HC1", data = reg_df)
gini_ivm08 <- feols(gini_norm ~ 1 | eci_software_norm + log_gdp_ppp_pc + eci_trade_norm + eci_tech_norm + eci_research_norm + log_pop + log_nat_res ~ sim_eci_software_norm + log_gdp_ppp_pc + log_gdp_ppp_pc2 + eci_trade_norm + eci_tech_norm + eci_research_norm + log_pop + log_nat_res, vcov = "HC1", data = reg_df)
...
# Gini regressions
reg_df <- subset(df, year==2020)
key_columns <- c("gini_norm", "log_gdp_ppp_pc", "log_gdp_ppp_pc2", "eci_software_norm", "eci_software075_norm", "eci_software125_norm", "eci_trade_norm", "eci_tech_norm", "eci_research_norm", "log_pop", "log_nat_res")
reg_df <- reg_df[complete.cases(reg_df[, ..key_columns]), ]
...
reg_df <- subset(df, year==2020)
key_columns <- c("log_emission_per_gdp", "log_gdp_ppp_pc", "eci_software_norm", "eci_software075_norm", "eci_software125_norm", "eci_trade_norm", "eci_tech_norm", "eci_research_norm", "log_pop", "log_nat_res")
reg_df <- reg_df[complete.cases(reg_df[, ..key_columns]), ]
...
### Table 1 -- GPD vs ECI -- for replication on new World Bank Data
reg_df <- subset(df, year==2020)
key_columns <- c("log_gdp_ppp_pc", "gini_norm", "log_emission_per_gdp", "eci_clusters_norm", "eci_software_norm", "eci_trade_norm", "eci_tech_norm", "eci_research_norm", "log_pop", "log_nat_res")
reg_df <- reg_df[complete.cases(reg_df[, ..key_columns]), ]


gdp_m01_clusters <- feols(log_gdp_ppp_pc ~ eci_clusters_norm + log_pop + log_nat_res, vcov = "HC1", data = reg_df)
gdp_m08_clusters <- feols(log_gdp_ppp_pc ~ eci_clusters_norm + eci_trade_norm + eci_tech_norm + eci_research_norm + log_pop + log_nat_res, vcov = "HC1", data = reg_df)
gini_m01_clusters <- feols(gini_norm ~ eci_clusters_norm + log_gdp_ppp_pc + log_pop + log_nat_res, vcov = "HC1", data = reg_df)
gini_m08_clusters <- feols(gini_norm ~ eci_clusters_norm + log_gdp_ppp_pc + eci_trade_norm + eci_tech_norm + eci_research_norm + log_pop + log_nat_res, vcov = "HC1", data = reg_df)
em_m01_clusters <- feols(log_emission_per_gdp ~ eci_clusters_norm + log_gdp_ppp_pc + log_pop + log_nat_res, vcov = "HC1", data = reg_df)
em_m08_clusters <- feols(log_emission_per_gdp ~ eci_clusters_norm + log_gdp_ppp_pc + eci_trade_norm + eci_tech_norm + eci_research_norm + log_pop + log_nat_res, vcov = "HC1", data = reg_df)
...
### ECI software -- comparison of different versions
df <- create_baseline_table(
  main_input_path = "../outputs/eci_regression_table.csv",
  iv_input_path = "../outputs/si_eci_software_2020_2023_ivreg.csv"
)
...
# ECI software -- co-occurrence clusters
eci_clusters2 <- fread("../outputs/eci_clusters_cooc_2020_2023.csv") %>%
  dplyr::select(iso2_code, year, eci) %>%
  unique() %>%
  rename(eci_clusters_cooc = eci) %>%
  group_by(year) %>%
  mutate(eci_clusters_cooc_norm = scale(eci_clusters_cooc)) %>%
  data.table()
...
key_columns1 <- c("log_gdp_ppp_pc", "eci_software_norm", "eci_clusters_theory_norm", "eci_clusters_cooc_norm", "eci_topic_norm", "log_pop", "log_nat_res")
reg_df1 <- reg_df[complete.cases(reg_df[, ..key_columns1]), ]

reg_df <- subset(df, year==selected_year)
key_columns2 <- c("gini_2020_2022_norm", "eci_software_norm", "eci_clusters_theory_norm", "eci_clusters_cooc_norm", "eci_topic_norm", "log_pop", "log_nat_res")
reg_df2 <- reg_df[complete.cases(reg_df[, ..key_columns2]), ]

reg_df <- subset(df, year==selected_year)
key_columns3 <- c("log_emission_per_gdp", "eci_software_norm", "eci_clusters_theory_norm", "eci_clusters_cooc_norm", "eci_topic_norm", "log_pop", "log_nat_res")
reg_df3 <- reg_df[complete.cases(reg_df[, ..key_columns3]), ]


gdp_m01_topic <- feols(log_gdp_ppp_pc ~ eci_software_norm + eci_clusters_theory_norm + eci_clusters_cooc_norm + eci_topic_norm + log_pop + log_nat_res, vcov = "HC1", data = reg_df1)
gini_m01_topic <- feols(gini_2020_2022_norm ~ eci_software_norm + eci_clusters_theory_norm + eci_clusters_cooc_norm + eci_topic_norm + log_gdp_ppp_pc + log_pop + log_nat_res, vcov = "HC1", data = reg_df2)
em_m01_topic <- feols(log_emission_per_gdp ~ eci_software_norm + eci_clusters_theory_norm + eci_clusters_cooc_norm + eci_topic_norm + log_gdp_ppp_pc + log_pop + log_nat_res, vcov = "HC1", data = reg_df3)


etable(
  gdp_m01_topic, gini_m01_topic, # em_m01_topic,
  digits = 5,
  digits.stats = 3,
  signif.code = c("***"=0.01, "**"=0.05, "*"=0.1),
  tex = FALSE
...
# GDP separate models
gdp_comp_m1 <- feols(log_gdp_ppp_pc ~ eci_software_norm + log_pop + log_nat_res, vcov = "HC1", data = reg_df1)
gdp_comp_m2 <- feols(log_gdp_ppp_pc ~ eci_clusters_theory_norm + log_pop + log_nat_res, vcov = "HC1", data = reg_df1)
gdp_comp_m3 <- feols(log_gdp_ppp_pc ~ eci_clusters_cooc_norm + log_pop + log_nat_res, vcov = "HC1", data = reg_df1)
gdp_comp_m4 <- feols(log_gdp_ppp_pc ~ eci_topic_norm + log_pop + log_nat_res, vcov = "HC1", data = reg_df1)
gdp_comp_m5 <- feols(log_gdp_ppp_pc ~ eci_software_norm + eci_clusters_theory_norm + eci_clusters_cooc_norm + eci_topic_norm + log_pop + log_nat_res, vcov = "HC1", data = reg_df1)
...
gini_comp_m1 <- feols(gini_2020_2022_norm ~ eci_software_norm + log_gdp_ppp_pc + log_pop + log_nat_res, vcov = "HC1", data = reg_df2)
gini_comp_m2 <- feols(gini_2020_2022_norm ~ eci_clusters_theory_norm + log_gdp_ppp_pc + log_pop + log_nat_res, vcov = "HC1", data = reg_df2)
gini_comp_m3 <- feols(gini_2020_2022_norm ~ eci_clusters_cooc_norm + log_gdp_ppp_pc + log_pop + log_nat_res, vcov = "HC1", data = reg_df2)
gini_comp_m4 <- feols(gini_2020_2022_norm ~ eci_topic_norm + log_gdp_ppp_pc + log_pop + log_nat_res, vcov = "HC1", data = reg_df2)
gini_comp_m5 <- feols(gini_2020_2022_norm ~ eci_software_norm + eci_clusters_theory_norm + eci_clusters_cooc_norm + eci_topic_norm + log_gdp_ppp_pc + log_pop + log_nat_res, vcov = "HC1", data = reg_df2)
...
#gini_ivm01 <- feols(gini_2020_2022_norm ~ 1 | eci_software_norm + log_gdp_ppp_pc + log_pop + log_nat_res ~ sim_eci_software_norm + log_gdp_ppp_pc + log_gdp_ppp_pc2 + log_pop + log_nat_res, vcov = "HC1", data = reg_df)
#gini_ivm08 <- feols(gini_2020_2022_norm ~ 1 | eci_software_norm + log_gdp_ppp_pc + eci_trade_norm + eci_tech_norm + eci_research_norm + log_pop + log_nat_res ~ sim_eci_software_norm + log_gdp_ppp_pc + log_gdp_ppp_pc2 + eci_trade_norm + eci_tech_norm + eci_research_norm + log_pop + log_nat_res, vcov = "HC1", data = reg_df)
